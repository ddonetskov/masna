...
g <- ggplot(data = data_pred, aes(x = age)) +
  geom_point(data = data2, aes(x = age, y = FVC, colour = names(colours)[1]), alpha = 0.5)
...
4. The data seems to have been collected over several years so it might be worth to take the time difference into the account. Checking the number of cases for different years per region.
